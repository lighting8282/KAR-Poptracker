...
kirby_air_ride_location = {}
kirby_air_ride_location.__index = kirby_air_ride_location
...
function kirby_air_ride_location.new(name)
    local self = setmetatable({}, kirby_air_ride_location)
    if name then
...
function kirby_air_ride_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = kirby_air_ride_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function kirby_air_ride_location:connect_two_ways(exit, rule)
...
function kirby_air_ride_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function kirby_air_ride_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function kirby_air_ride_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function kirby_air_ride_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function kirby_air_ride_location:accessibility()
...
-- 
function kirby_air_ride_location:discover(accessibility, keys)
...
entry_point = kirby_air_ride_location.new("entry_point")
